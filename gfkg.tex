...
\[
    (v + 0)(f) = v(f) + 0(f) = v(f)
\]
by defining $0: C^\infty(\mathbb{R}) \to \mathbb{R}$ to be the tangent vector that maps all functions to $0 \in \mathbb{R}$.
...
We have that
\[
    \gamma'(t): f \mapsto \frac{\d}{\d t} f\left( \gamma(t) \right).
\]
Notice that
\begin{align*}
...
\begin{align*}
    (\phi \circ \gamma)'(t)(f) &= \frac{\d}{\d t} f\left((\phi \circ \gamma)(t)\right) \\
        &= \frac{\d}{\d t} (f \circ \phi \circ \gamma)(t) \\
        &= \frac{\d}{\d t} (f \circ \phi) (\gamma(t)) \\
        &= \gamma'(t)(f \circ \phi) \\
        &= \gamma'(t)(\phi^* f) \\
        &= (\phi_* (\gamma'(t)))(f).
\end{align*}
...
Looking at the action of $\phi_* v$ on a function $f \in C^\infty (N)$,
\begin{align*}
...
\]
Consider $\phi_* \partial_x$,
\begin{align*}
    (\phi_* \partial_x)(f)(p) &= \partial_x u \cdot \partial_u f(u, v) + \partial_x v \cdot \partial_v f(u, v) \\
        &= \cos(\theta) \partial_u f(u, v) + \sin(\theta) \partial_v f(u, v),
\end{align*}
so
\[
    \phi_* \partial_x = \cos(\theta) \partial_u + \sin(\theta) \partial_v
\]
and similarly for $\phi_* \partial_y$,
\[
    \phi_* \partial_y = -\sin(\theta) \partial_u + \cos(\theta) \partial_v.
\]
But we can denote by $x$, $y$ the coordinates in the target manifold ($\mathbb{R}^2$ rotated by $\theta$, this is a similar abuse of notation as Solution~\ref{sol:pullbackexponential}) and therefore write\footnote{Note the sign difference.}
\begin{align*}
    \phi_* \partial_x &= \cos(\theta) \partial_x + \sin(\theta) \partial_y, \\
    \phi_* \partial_y &= -\sin(\theta) \partial_x + \cos(\theta) \partial_y.
\end{align*}
...
\begin{align*}
    \frac{\d}{\d t} f(\gamma(t)) &= \frac{\d}{\d t} f(x, y) \\
